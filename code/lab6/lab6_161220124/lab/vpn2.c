...
//		printf("recv ======== %d\n", recv);
	...
	/*	else
		...
		unsigned char address[4] = {10, 0, 0, 2};
		if(strncmp(iphead->dest_ip, device[0].local_ip_addr, 4) == 0)
		{
...
void repack(char *buffer, int buffer_len)
{
	...
	unsigned char dest_ip_addr[4] = {192, 168, 0, 2};
	int i;
	...
	iph->proto = 0x1;
	iph->head_checksum = 0;
	memcpy(iph->src_ip, device[0].local_ip_addr, 4);
...
void unpack(char *buffer, int buffer_len)
{
	...
	unsigned char dest_mac_addr[6];
	unsigned char dest_ip_addr[4] = {10, 0, 1, 2};
	int i;
	for(i = 0; i < MAX_ROUTE_INFO; i++)
	{
		if(strncmp(dest_ip_addr, route_info[i].destination, 3) == 0)
